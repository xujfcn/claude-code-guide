# 20｜Claude Code 接入 Crazyrouter 连载 20：找到好点子

> 本文是 Crazyrouter Claude Code 系列第 20 篇。本文会围绕「Claude Code 接入 Crazyrouter 连载 20：找到好点子」展开，重点覆盖 找到好点子、Step 1：建立判断标准 —— 什么样的需求用户愿意买单、开篇：独立开发程序员小明的故事。
>
...
> 确实，市面上很多编程课都是直接教你做项目：做个 Todo List、做个计算器、做个个人博客……这些项目确实能帮你熟悉语法和工具
...
2. 找到 10 个目标用户。问他们："你现在怎么解决这个问题？" 如果用户已经在用某种方法，说明问题确实存在。如果用户说不需要解决，那可能不是真需求。
3. 第二步：分析用户现有的替代方案，找出你的优势
4. 用户现在可能用其他产品、Excel、靠记忆，或者忍受着不解决。你需要弄清楚这些方案有什么缺点。你的产品要比它们好很多，用户才愿意换。
5. 第三步：测试用户是否愿意为你的产品付钱
6. 做预售或收定金。统计愿意付定金的用户比例（越早赚上钱说明需求越正确）：

   - 超过 10%：需求真实，值得投入
   - 5% 到 10%：需求存在，但需要打磨
   - 低于 5%：需求可能不成立
7. 第四步：估算这个市场有多大，能不能赚钱
8. 计算三个数字：目标用户总数 × 付费意愿 × 客单价。相乘后得到市场规模。如果市场太小，可能不值得做。
9. 第五步：思考你的产品有什么护城河，防止别人抄袭
10. 考虑这些壁垒：技术难度、网络效应、品牌、成本优势。这些能帮你长期保持竞争力。
...
AI 建议分三个阶段来做。第一阶段是 1-2 个月，目标是验证需求。不做 APP，先做一个轻应用，只提供产后专属课程（视频）和打卡社区。用人工方式提供动作指导，用户上传视频，教练回复。目标是验证产后妈妈是否愿意付费。

第二阶段是 3-6 个月，如果第一阶段验证成功，就增加核心功能。开发 AI 动作纠正，可以用现成的 SDK，例如成熟的视觉识别 API。增加个性化方案，优化社区功能。

第三阶段是 6-12 个月，规模化。开发独立 APP，与产后康复机构合作，拓展到孕期、备孕期。

关于技术实现，AI 建议 AI 动作纠正使用现成的 AI 能力，例如成熟的视觉识别 API，不需要从零开发，降低成本。前期用人工加 AI 混合模式，逐步过渡。
...
最推荐的是小红书。搜索"产后恢复"、"腹直肌分离"等关键词，找到相关博主，站内消息合作。用"免费体验 1 个月"的方式换取推广，成本几乎为 0。

其次是妈妈群。搜索社群、QQ 群，以"产后恢复经验分享"的名义进群，分享干货内容，不要直接广告，逐步建立信任。
...
第一阶段是 1-2 个月：做一个轻应用，找 2-3 位产后康复教练合作（分成模式），只提供产后专属课程（视频）和打卡社区，用人工方式提供动作指导。目标是 100 个用户，10% 付费转化率。
...
- 上一篇：[第 19 篇](./19-19-claude-code-接入-crazyrouter-连载-19-学会-ai-编程工具.md)
- 下一篇：[第 21 篇](./21-21-claude-code-接入-crazyrouter-连载-21-动手做出原型.md)
